...
compareFiles() {
   ...
   scoreLog=$4
   if [[ -e $stuLog ]]; then
      java -jar LogComparator.jar $seqNum $refLog $stuLog > $scoreLog
   else
...
#  Compile your solution:
   ...
   if [[ ! -e "./Minor/J3/DS/prQuadTree.class" ]]; then
      echo "Could not find prQuadTree.class."
      exit -1
   fi
   if [[ ! -e "./Minor/J3/DS/Compare2D.class" ]]; then
      echo "Could not find Compare2D.class."
      exit -1
   fi
   if [[ ! -e "./Minor/J3/DS/Direction.class" ]]; then
      echo "Could not find Direction.class."
      exit -1
   fi
   if [[ ! -e "./Minor/J3/Client/Point.class" ]]; then
...
#  Run comparison tool on the log files:
   ...
   echo >> $testLog
   compareFiles 1 "profTestTreeInitialization.txt" "TestTreeInitialization.txt" "initScore.txt"
   compareFiles 2 "profTestInsertion.txt" "TestInsertion.txt" "insertionScore.txt"
   compareFiles 3 "profTestRegionSearch.txt" "TestRegionSearch.txt" "regionScore.txt"
   compareFiles 4 "profTestDeletion.txt" "TestDeletion.txt" "deletionScore.txt"
